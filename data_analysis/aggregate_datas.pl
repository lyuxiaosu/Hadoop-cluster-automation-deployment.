...
sub create_clusterSlave_hardware {
	...
	open (fr_hardware, "< cluster_hardware_configure.txt") or die "cluster_hardware_configure.txt fail, $!";
	readline fr_hardware;

 	while($line=<fr_hardware>) {
		$s = index($line, ' ');
		$cluster_slave = substr($line, 0, $s);
		$log_data = substr($line, $s+1);
		chomp($log_data);
		$clusterSlave_hardware{$cluster_slave} = $log_data;
	}	
	close fr_hardware;
}
