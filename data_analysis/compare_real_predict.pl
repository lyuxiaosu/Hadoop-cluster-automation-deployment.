...
print fw "real_time(ms),predict_time(ms),application,cpu_frequency,memory(G),disk_speed(Mb/s),cluster\n";
...
while(my $line=<fr_real>) {
	...
	print fw "$real_time,$predict_time,$application,$cpu_frequency,$memory,$disk_speed,$cluster_i\n";
	$index++;
}
